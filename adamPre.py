...
class AdamPre(Optimizer): #for Prediction step method (Forked from author) with additional delta factor
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, name='NotGiven', d=1):
        self.name = name
        self.d = d
        ...
        for group in self.param_groups:
            for p in group['params']:
                if p.grad is None:
                    continue
                state = self.state[p]
                temp_grad = p.data.sub(state['oldWeights']).mul(self.d)
